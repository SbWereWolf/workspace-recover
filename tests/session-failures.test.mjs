...
async function fixture(t) {
 ...
 const manifest={schema:'workspace-recover/manifest/v3',name:'session-test',backup:{source:{path:source,exclude:[]},provider:{type:'local-files',root:path.join(root,'objects')}},restore:{existingTarget:'reject',workflow:[]},handoff:{provider:{type:'local-files',root:path.join(root,'objects')},subject:'rebranding'}};
...
test('unsafe transport filenames are rejected before any provider call',async t=>{
 ...
 const recovery={schema:'workspace-recover/recovery-manifest/v3',transport:{schema:'workspace-recover/transport-manifest/v3',provider:{type:'local-files'},archive:{fileName:'backup.tar.gz'},parts:[{index:0,fileName:'../../escape',bytes:1,sha256:'0'.repeat(64),remote:{id:'unknown'}}]},restore:{existingTarget:'reject',workflow:[]}};
