...
036 / 0.3.2: реализованы парные exec-профили упаковки/распаковки, точный NUL-список, проверка восстановленного состава до workflow и отдельно доставляемый bootstrap. Внешняя доставка подтверждается собственным receipt.
...
## 037 — повторная проверка поставки и интеграция среды

В 0.3.2 выявлена гонка теста таймаута: при параллельной нагрузке сигнал SIGTERM
мог приходить раньше установки обработчика в дочернем Node. Исправлена только
синхронизация проверки: реальный дочерний процесс подтверждает готовность, затем
управляемые часы родителя проходят исходные 150 и 250 мс. Проверка SIGKILL и
уведомительной семантики сохранена. Ошибочный исходный прогон сохранён отдельно.
Выпуск 0.3.3 не меняет runtime и формат v3.
