...
export function googleProfileDir(profile = 'default') {
  return path.join(homeConfigDir(), 'google-workspace', profile);
}
...
export async function authorizeGoogleWorkspace({ clientFile, profile = 'default', scopes = GOOGLE_SCOPES, noBrowser = false }) {
  const dir = googleProfileDir(profile);
  await ensureDir(dir);
  if (clientFile) {
